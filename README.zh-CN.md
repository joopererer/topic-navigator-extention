...
- 自定义 Onyx 主机匹配模式
- 界面语言偏好
- 外观样式设置（若启用配色预设 UI，还会保存最多 3 个配色槽位）
- 对话区字体缩放比例

以上数据保存在 **`chrome.storage.sync`**（同步存储区）：在浏览器已登录账号的情况下，**有可能随浏览器配置文件同步**。**不会**有意将对话正文上传到外部服务器。

### 隐私

- 不包含分析 / 统计埋点（No analytics）
- 无远程遥测（No remote telemetry）
- 扩展代码不发起对外部业务 API 的请求（No external API calls）
- 仅在浏览器扩展存储中保存偏好设置
- 不会故意将会话数据传输到当前设备以外
