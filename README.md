...
**Topic Navigator** is a Chromium extension that adds a turn-by-turn timeline and outline panel on supported AI chat sites (ChatGPT, Gemini, Claude, Onyx, etc.), with customizable rail/dot styling and optional self-hosted Onyx URL patterns. Build with `npm install && npm run build`, load the **`dist`** folder as an unpacked extension. Licensed under the **MIT License**.
